...
switch($_SESSION["Level"]) {
	...
	case "F" :
		...
		$sql2 = "SELECT B.univsno, A.studentid FROM examstudent as A, userinfo as B WHERE A.studentid = B.myid AND a.examid = " . $_GET["examID"] . " AND B.univid='" . $_SESSION["UID"] . "' ORDER BY B.univsno";
		break;
}
if(!$DB[0]->query($sql2)) {
...
while($ans = $DB[0]->fetch()) {
	...
	for($i = 0; $i < count($examItems); $i++) {
		$sql3 = "SELECT answered FROM examsubmit WHERE examid = " . $_GET["examID"] . " AND itemid = " . $examItems[$i] . " AND userid = '" . trim($findStd). "'";
		if(!$DB[1]->query($sql3)) {
		...
		if($DB[1]->getResult(0,0)) {
			$prtItem = $DB[1]->getResult(0,0) . "\t";
		}
		$prtItemStr .= $prtItem;
	}
	$prtItemStr .= "\n";
