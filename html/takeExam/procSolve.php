...
Desc.
	...
	- $_POST["getOptType"]
	- $_POST["getOptNo"]
Added. 2008. 6. 13 / 사용자가 입력한 답안을 examsubmit 에 저장한다.
		     변수 answeredStr을 저장한다.
============================================================================= */
...
if($_POST["getOptType"] != "R") {
	if( strtolower(trim($result[0])) == strtolower(trim($_POST["smtOption"])) )
		$isCorrect = 1;
		$answeredStr = trim($_POST["smtOption"]);
} else {
	$corAr = explode(",", $result[0]);
	$answeredStr = implode(",", $_POST["smtOption"]);
...
// DB에 저장
$sql = "INSERT INTO ExamSubmit VALUES (nextval('examsubmit_myid_seq'::regclass), " . $_POST["examID"] . ", '" . $_SESSION["ID"] . "', " . $_POST["myItem"] . ", '" . $isCorrect . "', CURRENT_TIMESTAMP, '" . $answeredStr ."')";
if(!$DB->query($sql)) {
