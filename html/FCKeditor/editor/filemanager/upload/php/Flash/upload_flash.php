...
여기까지 원본 */
// 여기서 부터 원본을 BIO 가 고친거

$myMicrotime = substr(microtime(),strrpos(microtime(),".")+1);
$sFile = str_replace(" ","",$myMicrotime);
$sFileName = trim($sFile).trim(substr($oFile['name'], strrpos($oFile['name'],".")));

//if(substr(strrchr(urlencode($oFile['name']),"%"),1) == "" ){
//        $sFileName = $oFile['name'];
//}else{
//        $sFileName = substr(strrchr(urlencode($oFile['name']),"%"),1);
//}
